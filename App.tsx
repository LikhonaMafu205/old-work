...
const [filteredList, setFilteredList] = useState<StoreItem[]>(items);
...
function handleDelete(id:string) {
  ...
  const updatedList = items.filter(
    // running through each item adding every item except the one matching id we want to delete
    (item) => item.id !== id
  );
...
useEffect(() => {
  // if there is nothing in the box, we reset the list of filtered items to match the list of ALL items
  if (searchTerm.trim() ==="") {
  setFilteredList(items);
  // otherwise, we filter the list based on the search term
  }else {
    //convert the search term to all lower case (to avoid issues with case sensitivity)
    const lower = searchTerm.toLowerCase();
    //update the filtered list
    setFilteredList(
      //To be filtered version of the main list
      items.filter(
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
      placeholder='Enter a name to search by...'
      value={searchTerm}
      ...
      <FlatList
        ...
        renderItem={({item}) => (
          <View>
            ...
            <Button title='Remove Item' onPress={() => handleDelete(item.id)} />
          </View>
        )}
        />
